...
def is_duplicated_number(three_digit):
    ...
    td = set([three_digit // 100, three_digit % 100 // 10,
        three_digit % 100 % 10])
    if len(td) is 3:
        result = False
    else:
...
def is_validated_number(user_input_number):
    ...
    if is_digit(user_input_number) is False:
        result = False
    elif is_duplicated_number(int(user_input_number)) is True:
        result = False
    elif is_between_100_and_999(int(user_input_number)) is False:
        result = False
    else:
        result = True
    # ==================================
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    user_input_number = ([user_input_number // 100,
        user_input_number % 100 // 10, user_input_number % 100 % 10])
    random_number = ([random_number // 100, random_number % 100 // 10,
        random_number % 100 % 10])
    for i in range(0, 3):
...
def main():
    ...
    while True:
        if sb[0] is 3:
            q = input('You win, one more(Y/N) ?')
            if is_yes(q) is True:
                sb = [0, 0]
                random_number = str(get_not_duplicated_three_digit_number())
                print("Random Number is : ", random_number)
                continue
            elif is_no(q) is True:
                break
            else:
        ...
        print('Strikes : %s , Balls: %s' % (sb[0], sb[1]))
    # ==================================
