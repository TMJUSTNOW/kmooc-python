...
def get_stock_data(url_address):
    ...
    # ===Modify codes below=============
    result = []
    for line in stock_data_string.split('\n'):
...
def get_attribute_data(stock_data, attribue, year=None, month=None):
    ...
    attribue_index = header.index(attribue)
    result = [[header[0], header[attribue_index]]]
    for item in stock_data[1:]:
        item_date, item_month, item_year = item[0].split('-')
        item_year = int(item_year) + 2000
        item_month = int(monthletter_dict.get(item_month))
        if year != None and month != None:
            if item_month != month or item_year != year: continue
        elif year != None and month == None:
            if item_year != year: continue
        result.append([item[0], item[attribue_index]])
    # ==================================
    return result
...
def get_average_value_of_attribute(stock_data, attribue, year=None, month=None):
    ...
    result = None
    # ===Modify codes below=============
    stock_data = get_attribute_data(stock_data, attribue, year, month) 
    f_value = [ item[1] for item in stock_data[1:] ]
    result = sum(list(map(float, f_value))) / len(f_value)
    # ==================================
    return result
...
def main():
    ...
    # ===Modify codes below=============
    while True:
        user_input= input('Insert Query Command - ex) SAMSUNG, 2014-12 Open, ALL : ')
        if user_input == '0': break
        query = separate_user_query(user_input)
        query_year, query_month = query[1].split('-')
        query_type = query[2]
        stock_data = get_stock_data(url)
        stock_headr = get_header_data(stock_data)
        stock_filtered = get_attribute_data(
            stock_data, query_type,
            year=int(query_year), month=int(query_month))
        if query[3].upper() == 'ALL':
            for item in stock_filtered: print(item[0], item[1])
        elif query[3].upper() == 'MEAN':
            stack_avg = get_average_value_of_attribute(
                stock_data, query_type,
                year=int(query_year), month=int(query_month))
            print(stack_avg)
        elif query[3].upper() == 'FILE':
            write_csv_file_by_result(stock_filtered, query[4])
            print('%s file created' % query[4])
    print('Good Bye')
    # ==================================
